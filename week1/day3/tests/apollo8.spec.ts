...
test("View Reports", async ({ page }) => {
  ...
  await page.getByRole('heading', { name: 'View Reports in My Orders' }).click();
...
await verifyBtn.click();
  ...
  await page.waitForURL(/test-orders/, { timeout: 20000 }); 
  await page.waitForTimeout(5000);
  console.log("Current URL:", page.url());
  await expect(page).toHaveURL(/test-orders/);
